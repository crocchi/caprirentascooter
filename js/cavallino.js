        ...
        async function initMap() {
          ...
          //libreria per i commenti
          const {Place} = await google.maps.importLibrary("places");
        
          // Use a place ID to create a new Place instance.
        const place = new Place({
          id: "ChIJoSU3AdV5OxMRtuRaWPW3-38", // caprirentscooter capri
        });
        ...
        const buildPhotoUri = (photo, options) => {
          ...
          return photo.uri || photo.photoUri || '';
        };

        const renderPlacePhotos = (photos) => {
          if (!photosSection || !photosGallery || !Array.isArray(photos) || photos.length === 0) {
            if (photosSection) {
          ...
          photos.slice(0, 8).forEach((photo, index) => {
            const fullSizeUri = buildPhotoUri(photo, { maxWidth: 1600, maxHeight: 1600 });
            const previewUri = buildPhotoUri(photo, { maxWidth: 900, maxHeight: 900 }) || fullSizeUri;
            ...
            photoCard.className = 'google-photo-card';
            photoCard.href = fullSizeUri || previewUri;
            photoCard.target = '_blank';
            photoCard.rel = 'noopener noreferrer';
            photoCard.setAttribute('aria-label', `Apri foto cliente ${index + 1}`);

            const photoImage = document.createElement('img');
            photoImage.src = previewUri;
            photoImage.alt = `Foto cliente Capri Rent A Scooter ${index + 1}`;
          ...
          if (!photosGallery.children.length) {
            photosSection.classList.add('is-hidden');
          }
        };
        ...
        try {
          ...
          //console.log(review);
            ...
            let tempEl=document.getElementById('slide-comments');
            //slide-comments
           // Format the review using HTML.
        
           /*
           tempEl.innerHTML += `
                            <div class="swiper-slide" data-swiper-slide-index="${cont}">
                                <div class="image-wrapper">
                                        <img class="img-fluid" src="${authorAttribution.photoURI}" alt="alternative">
                                </div>
                                        <div class="text-wrapper">
                                            <div class="testimonial-text">${text}</div>
                                            <div class="testimonial-author">${authorAttribution.displayName} - </div>
                                        </div> 
                            </div> 
                   `;*/
            ...
            //swiperSlide.setAttribute('data-swiper-slide-index', cont);
            ...
            img.src = authorAttribution.photoURI || 'images/logo2(0).gif';
            img.alt = 'alternative';
        
            // Append img to image-wrapper
            ...
            // Create the testimonial-text div
            ...
            // Create the testimonial-author div
            const testimonialAuthor = document.createElement('div');
            testimonialAuthor.className = 'testimonial-author';
            testimonialAuthor.textContent = `${authorAttribution.displayName} - ${relativePublishTimeDescription}`;
        
            //relativePublishTimeDescription
               // Create the star image element
               const starImg = document.createElement('img');
            starImg.className = 'img-star';
            starImg.src = `images/star${rating}.gif`;
        
        
            // Append testimonial-text and testimonial-author to text-wrapper
            textWrapper.appendChild(testimonialText);
            textWrapper.appendChild(testimonialAuthor);
            textWrapper.appendChild(starImg);
        
            // Append image-wrapper and text-wrapper to swiper-slide
            swiperSlide.appendChild(imageWrapper);
            swiperSlide.appendChild(textWrapper);
        ...
        //funzione x ricevere url della foto...altrimenti da errore di permesso
        let iii=Math.floor(Math.random() * 10) + 1;
        
        //click - change img src
        let cont=1;
        photoImg.addEventListener("click", (i) => {
            document.getElementById('image-contan').src = place.photos[cont++].getURI({maxHeight: 300});
          });
          photoImg.src = place.photos[iii].getURI({maxHeight: 300});
        */
        
        
          //google.maps.places.Pl
